...
| **Módulo 71 — Ponto de Venda** (`@alsham/pdv` + `0086_pdv.sql`) | ✅ **CONSTRUÍDO na Onda Dezoito (Fase 2 — ABRE o Vertical 🛒 Varejo & Supermercados)** — **arquivo, ainda não aplicado** (runbook §31). ⭐ A **venda comercial** (o cupom), NÃO o documento fiscal — NF-e/NFC-e é integração (Lei 3); o bastão é a decisão de dono explícita para o PDV que "integra-se por padrão". Cabeçalho + itens (FK intra-schema); finalizar CONGELA; `draft → completed/cancelled` TERMINAIS — **o DIVERGE do `rfq`**, que tem o meio-termo `open` (a venda fecha na hora). *Promoções* dobrada como campo de desconto simples; o total é VIEW. Cliente/produto por id solto. `consumes` vazio ([MODULO-PDV-SPEC](MODULO-PDV-SPEC.md)) |
| **Módulo 72 — Catálogo de Produtos** (`@alsham/catalog` + `0087_catalog.sql`) | ✅ **CONSTRUÍDO na Onda Dezoito (Vertical Varejo & Supermercados)** — **arquivo, ainda não aplicado** (runbook §31). O cadastro do que a loja vende: SKU TEXTO LIVRE opcional, nome, preço de tabela (valor + moeda, `>= 0`). ⭐ `active ↔ archived` — o produto descontinuado que volta é o MESMO (a física do `vendor`, o DIVERGE do `hr`). O preço efetivo da venda vive no item do cupom (`pdv`). `consumes` vazio ([MODULO-CATALOG-SPEC](MODULO-CATALOG-SPEC.md)) |
| **Módulo 73 — Sessão de Caixa** (`@alsham/cashregister` + `0088_cashregister.sql`) | ✅ **CONSTRUÍDO na Onda Dezoito (Vertical Varejo & Supermercados)** — **arquivo, ainda não aplicado** (runbook §31). O turno físico de uma gaveta: abre contando o fundo, fecha contando a gaveta. `open → closed`, `closed` TERMINAL — ⭐⭐ **o DIVERGE do `cash`**, que é livro-caixa perpétuo, imutável e SEM ciclo. UMA sessão aberta por caixa (constraint). A quebra de caixa (esperado × contado) é de tela. `consumes` vazio ([MODULO-CASHREGISTER-SPEC](MODULO-CASHREGISTER-SPEC.md)) |
| **Módulo 74 — Fidelidade** (`@alsham/loyalty` + `0089_loyalty.sql`) | ✅ **CONSTRUÍDO na Onda Dezoito (Vertical Varejo & Supermercados)** — **arquivo, ainda não aplicado** (runbook §31). O livro de pontos IMUTÁVEL (a física do `timesheet`). ⭐⭐ **O DIVERGE do `pcost`/`timesheet`:** a direção mora no `entry_type` (earn/redeem — a sinal do tipo do `cash`), `points > 0` sempre; o saldo é VIEW (Σ earn − Σ redeem). ⭐⭐ **Resgatar mais que o saldo é RECUSADO** — a terceira resposta, a física do `invest`. Cliente por id solto ao `crm` (obrigatório). `consumes` vazio ([MODULO-LOYALTY-SPEC](MODULO-LOYALTY-SPEC.md)) |
| **Módulo 75 — Risco Corporativo** (`@alsham/erisk` + `0090_erisk.sql`) | ✅ **CONSTRUÍDO na Onda Dezenove (Fase 3 — ABRE o Domain 🏛 GRC)** — **arquivo, ainda não aplicado** (runbook §32). ⭐⭐ **O DIVERGE do `risk` (Módulo 60, PMO):** o risco ESTRATÉGICO do negócio — SEM `project_id`. MANTIDO do `risk`: régua 1–5 (CHECK), severidade é leitura (a *Matriz de riscos*), e o ciclo — `mitigated` REABRE, `closed` TERMINAL. DIVERGE: `treatment` (accept/mitigate/transfer/avoid, os 4 T's da ISO 31000, CHECK), dono/categoria texto livre. `consumes` vazio ([MODULO-ERISK-SPEC](MODULO-ERISK-SPEC.md)) |
| **Módulo 76 — Controles Internos** (`@alsham/control` + `0091_control.sql`) | ✅ **CONSTRUÍDO na Onda Dezenove (Domain GRC)** — **arquivo, ainda não aplicado** (runbook §32). O cadastro de controles (tipo `preventive`/`detective`/`corrective` CHECK — física do COSO) + o livro IMUTÁVEL de testes (data, `pass`/`fail`, nota — a física do `timesheet`). `active ↔ archived`. Não é a política (`pol`), não é a auditoria (`audit`), não é o risco (`erisk`). `consumes` vazio ([MODULO-CONTROL-SPEC](MODULO-CONTROL-SPEC.md)) |
| **Módulo 77 — Canal de Denúncias** (`@alsham/whistle` + `0092_whistle.sql`) | ✅ **CONSTRUÍDO na Onda Dezenove (Domain GRC)** — **arquivo, ainda não aplicado** (runbook §32). ⭐⭐ **O ANONIMATO é físico:** se anônima, o denunciante NUNCA é gravado (gatilho descarta `auth.uid()` + CHECK + a política de leitura) — a única forma de nunca vazar é nunca ter. O relato nasce imutável; o tratamento anda (`open → under_review → resolved/dismissed`). Confidencialidade na RLS. `consumes` vazio ([MODULO-WHISTLE-SPEC](MODULO-WHISTLE-SPEC.md)) |
| **Módulo 78 — Gestão de Vulnerabilidades** (`@alsham/vuln` + `0093_vuln.sql`) | ✅ **CONSTRUÍDO na Onda Dezenove (Fase 3 — ABRE o Domain 🔐 Segurança da Informação)** — **arquivo, ainda não aplicado** (runbook §32). A vulnerabilidade dos sistemas DO TENANT (IAM/Cofre/SIEM/Backup da plataforma ficam FORA) — a identidade do `nc`/`capa`: severidade 1–5 CHECK, `open → in_progress → remediated/accepted_risk` (as DUAS respostas terminais, com justificativa). `consumes` vazio ([MODULO-VULN-SPEC](MODULO-VULN-SPEC.md)) |
| **Módulo 79 — Resposta a Incidentes** (`@alsham/secincident` + `0094_secincident.sql`) | ✅ **CONSTRUÍDO na Onda Dezenove (Domain Segurança da Informação)** — **arquivo, ainda não aplicado** (runbook §32). ⭐⭐ **O DIVERGE do `occ`:** timeline NIST de 5 estados (`detected → contained → eradicated → recovered → closed`) vs 1 par do `occ`; editável-enquanto-aberto e congela-no-fim (a física do `risk`) vs imutável-do-nascimento. Campos próprios: `attack_vector`, `affected_data`, `severity` 1–5. A resposta é livro imutável de atos (o MANTIDO). `consumes` vazio ([MODULO-SECINCIDENT-SPEC](MODULO-SECINCIDENT-SPEC.md)) |
| **Módulo 80 — Continuidade de Negócios** (`@alsham/continuity` + `0095_continuity.sql`) | ✅ **CONSTRUÍDO na Onda Dezenove (Domain Segurança da Informação)** — **arquivo, ainda não aplicado** (runbook §32). O plano (`active ↔ archived`, RTO/RPO texto livre) + o livro IMUTÁVEL de drills (a física do `timesheet`) — a prova de que o plano funciona. O documento detalhado do plano é o `pol` (declarado FORA); os drills justificam o módulo. `consumes` vazio ([MODULO-CONTINUITY-SPEC](MODULO-CONTINUITY-SPEC.md)) |
| **Módulo 81 — Usinas (e Geração distribuída)** (`@alsham/plant` + `0096_plant.sql`) | ✅ **CONSTRUÍDO na Onda Vinte (Fase 3 — ABRE o Vertical ☀️ Energia)** — **arquivo, ainda não aplicado** (runbook §33). ⭐⭐ **UM módulo, DUAS capacidades:** Usinas E Geração distribuída — na física o MESMO objeto, distinguido por `plant_type` TEXTO LIVRE (nunca enum — a consolidação do `esg`/`idea`/`ip`). `capacity_kwp > 0`; `active ↔ archived` (a usina que volta a operar é a MESMA — a física do `catalog`/`vendor`). Manutenção→`mnt`, Contrato→`ctr` declarados FORA. `consumes` vazio ([MODULO-PLANT-SPEC](MODULO-PLANT-SPEC.md)) |
| **Módulo 82 — Assinatura de Energia** (`@alsham/subscription` + `0097_subscription.sql`) | ✅ **CONSTRUÍDO na Onda Vinte (Vertical ☀️ Energia)** — **arquivo, ainda não aplicado** (runbook §33). O modelo central da Curva C solar: o consumidor assina uma FATIA (`allocation_percent` `0<x<=100`) da geração de uma usina (id solto ao `plant`), cliente por id solto ao `crm`. ⭐⭐ **Nasce ativa — SEM `pending`** (o intermediário seria viés de uma distribuidora); ⭐ `active → cancelled` **TERMINAL** — quem re-assina faz OUTRA (a física do `proj`, o DIVERGE consciente do `catalog`); cancelar exige razão + `.decide`. Desconto/faturamento FORA. `consumes` vazio ([MODULO-SUBSCRIPTION-SPEC](MODULO-SUBSCRIPTION-SPEC.md)) |
| **Módulo 83 — Monitoramento de Geração** (`@alsham/genreading` + `0098_genreading.sql`) | ✅ **CONSTRUÍDO na Onda Vinte (Vertical ☀️ Energia)** — **arquivo, ainda não aplicado** (runbook §33). ⭐ **Reaproveita a identidade do `esg`:** leitura periódica IMUTÁVEL (duas camadas), `generated_kwh >= 0` (zero é leitura real — à noite; o MANTIDO do `esg`), unidade TEXTO LIVRE, sem ciclo. ⭐ **O DIVERGE do `esg`:** a usina é OBRIGATÓRIA (`plant_id NOT NULL` — não há geração no ar), por id solto. Performance ratio/alerta FORA. `consumes` vazio ([MODULO-GENREADING-SPEC](MODULO-GENREADING-SPEC.md)) |
| **Módulo 84 — Créditos de Compensação** (`@alsham/creditbalance` + `0099_creditbalance.sql`) | ✅ **CONSTRUÍDO na Onda Vinte (Vertical ☀️ Energia — FECHA a onda)** — **arquivo, ainda não aplicado** (runbook §33). O livro de créditos do SCEE/ANEEL, a identidade do `loyalty`: direção no `credit_type` (generated/consumed), `quantity_kwh > 0`, saldo é VIEW. ⭐⭐ **Consumir > saldo é RECUSADO — a TERCEIRA resposta, por física PRÓPRIA** (não copiada do `loyalty`): crédito é energia realmente gerada, saldo negativo inventaria energia inexistente (a razão infísica do `esg`). Validade (60m)/abatimento na fatura FORA. `consumes` vazio ([MODULO-CREDITBALANCE-SPEC](MODULO-CREDITBALANCE-SPEC.md)) |
