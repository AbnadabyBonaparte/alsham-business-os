...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-IDEA-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'idea',"));
  if (!meu) return '';
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo idea contém ${needle}`);
  return JSON.parse(hit.slice(1, hit.lastIndexOf("'")));
}

describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `rnd`, ancorado na linha de Pesquisa & Desenvolvimento da Taxonomia', () => {
    const taxonomia = readFileSync(TAXONOMIA, 'utf8');
    assert.equal(MANIFEST.taxonomy.layer, 'domain');
    assert.equal((MANIFEST.taxonomy as { domain: string }).domain, 'rnd');
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Pipeline de inovação') && l.includes('Portfólio tecnológico'));
    assert.ok(linha, 'a linha de capacidades de Pesquisa & Desenvolvimento sumiu da Taxonomia');
    const listadas = linha!.split('·').map((c) => c.trim());
    for (const cap of MANIFEST.capabilities) {
      assert.ok(
        listadas.includes(cap.canonicalName),
        `${cap.canonicalName} não está entre as capacidades de P&D na Taxonomia`,
      );
    }
  });

  test('⭐ a chave de domínio bate com a store-taxonomy (a seção rnd existe)', () => {
    const store = readFileSync(STORE_TAX, 'utf8');
    assert.ok(store.includes("key: 'rnd'"), 'store-taxonomy.ts não tem a chave rnd');
  });

  test('⛔ o schema não cria enum de etapa — as etapas são texto livre (Lei das Etapas)', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+idea\./i);
    assert.match(migrationCode, /name\s+text\s+not null\s+check/i);
  });

  test('⭐⭐ o schema NÃO tem project_id (o DIVERGE do kanban)', () => {
    // Nenhuma COLUNA `project_id` (a prosa do comentário de schema menciona a
    // palavra; o que importa é não haver a coluna — o `promoted_project_id` é
    // outra coisa, o elo de destino legítimo).
    assert.doesNotMatch(migrationCode, /^\s*project_id\s+uuid/im);
    // mas TEM o elo de DESTINO, promoted_project_id.
    assert.match(migrationCode, /promoted_project_id/i);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0083_idea.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-IDEA-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('idea.idea.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('idea.idea.registered') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...codigo.matchAll(/\('(idea\.[a-z.]+)'\)/g)].map((m) => m[1]);
