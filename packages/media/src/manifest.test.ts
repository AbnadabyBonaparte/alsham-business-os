...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-MEDIA-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'media',"));
  assert.ok(meu, 'o seed não registra o módulo media');
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⭐ o Domain é `marketing` — e a capacidade *Mídia* está na linha', () => {
    ...
    assert.ok(linhaMkt.includes('Mídia'), 'a capacidade *Mídia* sumiu da linha — re-ancore');
  });

  /**
   * ⭐ ANTI-VIÉS com recusa nomeada: CATÁLOGO, não cofre — nada de
   * upload/storage (Core não construído), miniatura, gestão de direitos
   * ou busca de conteúdo — nem enum: tipo é texto livre e etiqueta é
   * tabela do tenant.
   */
  test('⛔ o schema não tem cofre, miniatura, direitos, busca — nem enum', () => {
    assert.doesNotMatch(migrationCode, /storage\.objects|bucket|upload/i);
    assert.doesNotMatch(migrationCode, /thumbnail|preview_url/i);
    assert.doesNotMatch(migrationCode, /copyright|license_|royalt/i);
    assert.doesNotMatch(migrationCode, /tsvector|to_tsquery|full_text/i);
    assert.doesNotMatch(migrationCode, /create\s+type\s+media\./i);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0041_media.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-MEDIA-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('media.asset.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('media.usage.recorded') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(media\.[a-z.]+)'\)/g)].map((m) => m[1]);
