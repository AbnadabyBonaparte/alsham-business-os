...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-PLANT-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'plant',"));
  assert.ok(meu, 'o seed não registra o módulo plant');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo plant contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é VERTICAL `energy`, ancorado na linha de Energia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Usinas') && l.includes('Comercialização e leads'));
    ...
    for (const cap of MANIFEST.capabilities) {
      assert.ok(
        ...
        `${cap.canonicalName} não está entre as capacidades de Energia na Taxonomia`,
      );
    }
  });
  ...
  test('⭐⭐ UM módulo, DUAS capacidades (a consolidação de Geração distribuída)', () => {
    const keys = MANIFEST.capabilities.map((c) => c.key);
    assert.ok(keys.includes('power-plants'), 'falta a capacidade Usinas');
    assert.ok(keys.includes('distributed-generation'), 'falta a capacidade Geração distribuída consolidada');
  });

  test('⛔ o schema não cria enum — o tipo é texto livre; a tabela de usinas existe', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+plant\./i);
    assert.match(migrationCode, /create\s+table\s+plant\.plants/i);
    assert.match(migrationCode, /check\s*\(\s*status in \('active', 'archived'\)\s*\)/i);
    assert.match(migrationCode, /capacity_kwp[\s\S]*?check\s*\(\s*capacity_kwp > 0\s*\)/i);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0096_plant.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-PLANT-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('plant.plant.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('plant.plant.registered') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...codigo.matchAll(/\('(plant\.[a-z.]+)'\)/g)].map((m) => m[1]);
