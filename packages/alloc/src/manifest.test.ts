...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-ALLOC-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'alloc',"));
  assert.ok(meu, 'o seed não registra o módulo alloc');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo alloc contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `pmo`, ancorado na linha de PMO & Projetos da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Recursos') && l.includes('Portfólio'));
  ...
  test('⛔ o schema não cria enum — o recurso e a régua não viram vocabulário fixo', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+alloc\./i);
    assert.match(migrationCode, /allocation_pct\s+numeric/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0071_alloc.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-ALLOC-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('alloc.allocation.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('alloc.allocation.registered') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(alloc\.[a-z.]+)'\)/g)].map((m) => m[1]);
