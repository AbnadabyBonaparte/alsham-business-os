...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-PCOST-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'pcost',"));
  assert.ok(meu, 'o seed não registra o módulo pcost');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo pcost contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⛔ o schema não cria enum de tipo — o custo é lançamento livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+pcost\./i);
    assert.match(migrationCode, /project_name\s+text/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0072_pcost.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-PCOST-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('pcost.entry.record') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('pcost.entry.recorded') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(pcost\.[a-z.]+)'\)/g)].map((m) => m[1]);
