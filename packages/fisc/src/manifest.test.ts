...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-FISC-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'fisc',"));
  assert.ok(meu, 'o seed não registra o módulo fisc');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo fisc contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é VERTICAL `government`, ancorado na linha de Governo', () => {
    ...
    assert.ok(linha!.includes('Fiscalização'), 'a Taxonomia de Governo deixou de listar Fiscalização');
  });

  test('⭐ o canonicalName da capacidade bate com a palavra da Taxonomia', () => {
    assert.equal(MANIFEST.capabilities[0]!.canonicalName, 'Fiscalização');
  });
  ...
  test('⛔ este módulo NÃO constrói o auto de infração — nenhuma tabela de penalidade/multa/enum nasce aqui (Lei 3)', () => {
    assert.doesNotMatch(migrationCode, /create\s+table\s+fisc\.(auto|autos|infrac\w*|penalt\w*|fines?|multas?)/i);
    assert.doesNotMatch(migrationCode, /create\s+type\s+fisc\./i);
  });

  test('⭐⭐ a vistoria (fisc.inspections) NÃO TEM coluna de status nem allowed_transition próprio', () => {
    const bloco = migrationCode.split('create table fisc.inspections')[1]?.split(');')[0] ?? '';
    assert.ok(bloco.length > 0, 'fisc.inspections não encontrada na migration');
    assert.doesNotMatch(bloco, /\bstatus\b/, 'a vistoria ganhou status — ela não tem ciclo de vida');
    assert.doesNotMatch(migrationCode, /fisc\.inspection_allowed_transition/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0108_fisc.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-FISC-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('fisc.target.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('fisc.inspection.recorded') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(fisc\.[a-z.]+)'\)/g)].map((m) => m[1]);
