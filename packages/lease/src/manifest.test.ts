...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-LEASE-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'lease',"));
  assert.ok(meu, 'o seed não registra o módulo lease');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo lease contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⛔ a migration NÃO reescreve o ctr: nenhuma tabela de vigência/reajuste/renovação', () => {
    // A vigência, o reajuste e a renovação são do ctr — nada disso nasce aqui.
    assert.doesNotMatch(migrationCode, /create\s+table\s+lease\.terms/i);
    assert.doesNotMatch(migrationCode, /create\s+table\s+lease\.adjustments/i);
    assert.doesNotMatch(migrationCode, /create\s+table\s+lease\.renewals/i);
    assert.doesNotMatch(migrationCode, /create\s+type\s+lease\./i);
    assert.match(migrationCode, /contract_id/);
    assert.match(migrationCode, /store_id/);
  });
  ...
  test('consumes é VAZIO — cruzar com ctr.contract.* é integração futura (Lei 7)', () => {
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0054_lease.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-LEASE-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('lease.agreement.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('lease.agreement.registered') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(lease\.[a-z.]+)'\)/g)].map((m) => m[1]);
