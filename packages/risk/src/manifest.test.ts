...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-RISK-SPEC.md');
...
// seed ganhar a linha do `risk`, os subtestes de TRANSCRIÇÃO abaixo falham DE
...
const blocoDoModulo: string | null = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'risk',"));
  if (!meu) return null;
  return meu.slice(0, meu.indexOf('on conflict'));
})();

function jsonBlockContaining(needle: string): unknown[] {
  assert.ok(blocoDoModulo, 'o seed não registra o módulo risk (cartão do orquestrador)');
  const blocks = blocoDoModulo.replace(/--[^\n]*/g, '').match(/'\[[\s\S]*?\]'::jsonb/g) ?? [];
  const hit = blocks.find((b) => b.includes(needle));
  assert.ok(hit, `nenhum bloco jsonb do módulo risk contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⛔ o schema não cria enum de tipo — descrição é texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+risk\./i);
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0075_risk.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-RISK-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('risk.entry.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('risk.entry.registered') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(risk\.[a-z.]+)'\)/g)].map((m) => m[1]);
