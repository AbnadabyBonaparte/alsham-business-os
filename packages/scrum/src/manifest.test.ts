...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-SCRUM-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'scrum',"));
  assert.ok(meu, 'o seed não registra o módulo scrum');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo scrum contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⛔ o schema não cria enum de tipo — nome/objetivo são texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+scrum\./i);
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0073_scrum.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-SCRUM-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('scrum.sprint.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('scrum.sprint.registered') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(scrum\.[a-z.]+)'\)/g)].map((m) => m[1]);
