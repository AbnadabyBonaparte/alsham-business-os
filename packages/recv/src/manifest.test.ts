...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-RECV-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'recv',"));
  assert.ok(meu, 'o seed não registra o módulo recv');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo recv contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `procurement`, ancorado na linha de Compras da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Recebimento') && l.includes('Estoque mínimo'));
  ...
  test('⛔ o schema não cria enum nem coluna de status — o recebimento é ato pontual', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+recv\./i);
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0060_recv.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-RECV-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('recv.receipt.record') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('recv.receipt.recorded') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(recv\.[a-z.]+)'\)/g)].map((m) => m[1]);
