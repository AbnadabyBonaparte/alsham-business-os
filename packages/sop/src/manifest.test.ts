...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-SOP-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'sop',"));
  assert.ok(meu, 'o seed não registra o módulo sop');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo sop contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `supply-chain`, ancorado na linha de Supply Chain da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('S&OP') && l.includes('Performance logística'));
  ...
  test('⛔ o schema não cria enum — e o vínculo com o plano é id solto (texto/uuid)', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+sop\./i);
    assert.match(migrationCode, /plan_id\s+uuid/);
  });
  ...
  test('⭐ há DUAS permissões: desenhar e aprovar (a separação de poderes)', () => {
    assert.deepEqual(
      MANIFEST.permissions.map((p) => p.key).sort(),
      ['sop.round.approve', 'sop.round.manage'],
    );
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0064_sop.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-SOP-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('sop.round.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('sop.round.registered') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(sop\.[a-z.]+)'\)/g)].map((m) => m[1]);
