...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-AUDIT-SPEC.md');
...
// ⚠️ O cartão do audit no seed é a ÚNICA peça que este módulo NÃO cria (o seed é
// arquivo compartilhado, editado pelo dono). Enquanto a Onda Quatorze não é
// sedimentada, o cartão não está no seed: os testes do describe "o seed
// transcreve" SKIPAM (nunca inventam um verde falso — Lei 7) até o cartão
// entrar, e os testes de contrato do manifesto passam. A leitura do bloco é LAZY.
const CARTAO_NO_SEED = sql.includes("'audit',");

function blocoDoModulo(): string {
  const inserts = sql.split(/insert into core\.module_registry/);
  const meu = inserts.find((b) => b.includes("'audit',"));
  assert.ok(meu, 'o seed não registra o módulo audit (cartão adicionado pelo dono)');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo audit contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `quality`, ancorado na linha de Qualidade da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Auditorias') && l.includes('CAPA'));
  ...
  test('⛔ o schema não cria enum de tipo — tipo/escopo são texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+audit\./i);
    assert.match(migrationCode, /scope\s+text/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0079_audit.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-AUDIT-SPEC.md não existe');
  });
});

describe('o seed transcreve o manifesto fielmente', () => {
  test('o seed registra este módulo, com esta versão, nome e resumo', { skip: !CARTAO_NO_SEED }, () => {
  ...
  test('⭐ a taxonomia DOMAIN é a mesma nos dois (domain_key)', { skip: !CARTAO_NO_SEED }, () => {
    const bloco = blocoDoModulo();
  ...
  test('as capacidades do seed são exatamente as do manifesto', { skip: !CARTAO_NO_SEED }, () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', { skip: !CARTAO_NO_SEED }, () => {
    const seeded = jsonBlockContaining('audit.audit.manage') as { key: string }[];
    assert.deepEqual(seeded.map((p) => p.key).sort(), MANIFEST.permissions.map((p) => p.key).sort());
  });

  test('os eventos emitidos do seed são exatamente os do manifesto', { skip: !CARTAO_NO_SEED }, () => {
    const seeded = jsonBlockContaining('audit.audit.scheduled') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(audit\.[a-z.]+)'\)/g)].map((m) => m[1]);
