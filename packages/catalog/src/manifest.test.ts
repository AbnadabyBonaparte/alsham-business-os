...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-CATALOG-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'catalog',"));
  assert.ok(meu, 'o seed não registra o módulo catalog');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo catalog contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é VERTICAL `retail`, ancorado na linha de Varejo & Supermercados', () => {
    ...
    for (const cap of MANIFEST.capabilities) {
      assert.ok(
        ...
        `${cap.canonicalName} não está entre as capacidades de Varejo & Supermercados na Taxonomia`,
      );
    }
  });
  ...
  test('⛔ o schema não cria enum — o SKU é texto livre; a tabela de produtos existe', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+catalog\./i);
    assert.match(migrationCode, /create\s+table\s+catalog\.products/i);
    assert.match(migrationCode, /check\s*\(\s*status in \('active', 'archived'\)\s*\)/i);
    assert.match(migrationCode, /price_cents\s+bigint[\s\S]*?check\s*\(\s*price_cents >= 0\s*\)/i);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0087_catalog.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-CATALOG-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('catalog.product.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('catalog.product.registered') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...codigo.matchAll(/\('(catalog\.[a-z.]+)'\)/g)].map((m) => m[1]);
