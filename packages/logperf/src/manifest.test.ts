...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-LOGPERF-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'logperf',"));
  assert.ok(meu, 'o seed não registra o módulo logperf');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo logperf contém ${needle}`);
  return JSON.parse(hit.slice(1, hit.lastIndexOf("'")));
}

describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `supply-chain`, ancorado na linha de Supply Chain da Taxonomia', () => {
    const taxonomia = readFileSync(TAXONOMIA, 'utf8');
    assert.equal(MANIFEST.taxonomy.layer, 'domain');
    assert.equal((MANIFEST.taxonomy as { domain: string }).domain, 'supply-chain');
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Performance logística') && l.includes('Abastecimento'));
    assert.ok(linha, 'a linha de capacidades de Supply Chain sumiu da Taxonomia');
    ...
    for (const cap of MANIFEST.capabilities) {
      assert.ok(
        ...
        `${cap.canonicalName} não está entre as capacidades de Supply Chain na Taxonomia`,
      );
    }
  });

  test('⭐ a chave de domínio bate com a store-taxonomy (a seção Supply Chain existe)', () => {
    const store = readFileSync(STORE_TAX, 'utf8');
    assert.ok(
      store.includes("key: 'supply-chain'"),
      'store-taxonomy.ts não tem a chave supply-chain — a seção Supply Chain não existiria',
    );
  });

  test('⛔ o schema não cria enum de status/nota — a nota é CHECK, não enum', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+logperf\./i);
    assert.match(migrationCode, /rating\s+int/);
  });

  test('🔴 a migration NÃO referencia o schema dc — o vínculo com o centro é id solto', () => {
    assert.doesNotMatch(migration, /\bdc\./, 'a migration não pode referenciar o schema dc — dc_center_id é id solto');
  });
  ...
  test('a permissão de registrar é a needle esperada — logperf.appraisal.record', () => {
    assert.ok(MANIFEST.permissions.some((p) => p.key === 'logperf.appraisal.record'));
    assert.match(migrationCode, /logperf\.appraisal\.record/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0067_logperf.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-LOGPERF-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('logperf.appraisal.record') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('logperf.appraisal.recorded') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(logperf\.[a-z.]+)'\)/g)].map((m) => m[1]);
