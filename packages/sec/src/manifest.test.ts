...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-SEC-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'sec',"));
  assert.ok(meu, 'o seed não registra o módulo sec');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo sec contém ${needle}`);
  return JSON.parse(hit.slice(1, hit.lastIndexOf("'")));
}

describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é VERTICAL `shopping-centers`, ancorado na linha de Shopping Centers', () => {
    const taxonomia = readFileSync(TAXONOMIA, 'utf8');
    assert.equal(MANIFEST.taxonomy.layer, 'vertical');
    assert.equal((MANIFEST.taxonomy as { vertical: string }).vertical, 'shopping-centers');
    const linha = taxonomia.split('\n').find((l) => l.includes('Lojistas') && l.includes('Contratos de locação'));
    assert.ok(linha, 'a linha de capacidades de Shopping Centers sumiu da Taxonomia');
    assert.ok(linha!.includes('Segurança'), 'a Taxonomia de Shopping Centers deixou de listar Segurança');
  });

  test('⭐ a chave vertical bate com a store-taxonomy (a pill gradua)', () => {
    const store = readFileSync(STORE_TAX, 'utf8');
    assert.ok(
      store.includes("key: 'shopping-centers'"),
      'store-taxonomy.ts não tem a chave shopping-centers — a pill não graduaria',
    );
  });

  test('⛔ este módulo NÃO recria o incidente — nenhuma tabela de ocorrência/enum nasce aqui', () => {
    assert.doesNotMatch(migrationCode, /create\s+table\s+sec\.(incidents|occurrences|events)/i);
    assert.doesNotMatch(migrationCode, /create\s+type\s+sec\./i);
  });

  test('⭐⭐ a ronda (sec.patrols) NÃO TEM coluna de status nem allowed_transition próprio', () => {
    const bloco = migrationCode.split('create table sec.patrols')[1]?.split(');')[0] ?? '';
    assert.ok(bloco.length > 0, 'sec.patrols não encontrada na migration');
    assert.doesNotMatch(bloco, /\bstatus\b/, 'a ronda ganhou status — ela não tem ciclo de vida');
    assert.doesNotMatch(migrationCode, /sec\.patrol_allowed_transition/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0057_sec.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-SEC-SPEC.md não existe');
  });

  test('⭐⭐ o contraste sec×occ: o occ TEM lifecycle de incidente; o sec não recria', () => {
    const occ = readFileSync(MIGRATION_OCC, 'utf8').replace(/--[^\n]*/g, '');
    assert.match(occ, /'open'\s*,\s*'closed'/, 'o occ perdeu o par open→closed — a âncora do contraste sumiu');
    assert.match(occ, /closed_at/);
    assert.match(occ, /outcome/);
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('sec.checkpoint.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('sec.patrol.recorded') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(sec\.[a-z.]+)'\)/g)].map((m) => m[1]);
