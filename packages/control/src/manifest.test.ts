...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-CONTROL-SPEC.md');
...
// seed ganhar a linha do `control`, os subtestes de TRANSCRIÇÃO abaixo falham DE
...
const blocoDoModulo: string | null = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'control',"));
  if (!meu) return null;
  return meu.slice(0, meu.indexOf('on conflict'));
})();

function jsonBlockContaining(needle: string): unknown[] {
  assert.ok(blocoDoModulo, 'o seed não registra o módulo control (cartão do orquestrador)');
  const blocks = blocoDoModulo.replace(/--[^\n]*/g, '').match(/'\[[\s\S]*?\]'::jsonb/g) ?? [];
  const hit = blocks.find((b) => b.includes(needle));
  assert.ok(hit, `nenhum bloco jsonb do módulo control contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⛔ o schema não cria enum de tipo — o tipo é CHECK, a descrição é texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+control\./i);
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0091_control.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-CONTROL-SPEC.md não existe');
  });
});

describe('⭐ o schema do 0091 tem as duas tabelas com a física certa', () => {
  test('control.controls existe com o tipo em CHECK argumentado', () => {
    assert.match(migrationCode, /create table control\.controls/i);
    assert.match(
      migrationCode,
      /control_type\s+text\s+not null\s+check\s*\(\s*control_type\s+in\s*\(\s*'preventive'\s*,\s*'detective'\s*,\s*'corrective'\s*\)\s*\)/i,
    );
  });

  test('control.tests existe com o resultado em CHECK (pass/fail)', () => {
    assert.match(migrationCode, /create table control\.tests/i);
    assert.match(
      migrationCode,
      /result\s+text\s+not null\s+check\s*\(\s*result\s+in\s*\(\s*'pass'\s*,\s*'fail'\s*\)\s*\)/i,
    );
  });

  test('⭐ o livro de testes NÃO tem grant de update/delete — só select, insert', () => {
    assert.match(migration, /grant\s+select,\s*insert\s+on\s+control\.tests\s+to\s+authenticated/i);
    assert.doesNotMatch(
      migration,
      /grant[^;]*\bupdate\b[^;]*on\s+control\.tests\s+to\s+authenticated/i,
    );
    assert.doesNotMatch(
      migration,
      /grant[^;]*\bdelete\b[^;]*on\s+control\.tests\s+to\s+authenticated/i,
    );
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('control.control.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('control.control.registered') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(control\.[a-z.]+)'\)/g)].map((m) => m[1]);
