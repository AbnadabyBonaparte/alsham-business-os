...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-CASHREGISTER-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'cashregister',"));
  assert.ok(meu, 'o seed não registra o módulo cashregister');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo cashregister contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é VERTICAL `retail`, ancorado na linha de Varejo & Supermercados', () => {
    ...
    // A capacidade deste módulo é uma das da linha (separadas por '·').
    const nomes = linha!.split('·').map((s) => s.trim());
    for (const c of MANIFEST.capabilities) {
      assert.ok(
        nomes.includes(c.canonicalName),
        `a capacidade ${c.canonicalName} não está na linha de Varejo da Taxonomia`,
      );
    }
  });
  ...
  test('⭐ é EXATAMENTE uma permissão: cashregister.session.manage', () => {
    assert.deepEqual(MANIFEST.permissions.map((p) => p.key), ['cashregister.session.manage']);
  });
  ...
  test('⭐ emite EXATAMENTE opened e closed (v1, passado)', () => {
    assert.deepEqual(
      MANIFEST.events.emits.map((e) => e.type).sort(),
      ['cashregister.session.closed', 'cashregister.session.opened'],
    );
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0088_cashregister.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-CASHREGISTER-SPEC.md não existe');
  });
});

describe('⭐ o schema da migration bate com a física do módulo', () => {
  test('tem a tabela cashregister.sessions', () => {
    assert.match(migrationCode, /create\s+table\s+cashregister\.sessions/i);
  });

  test('⭐ o CHECK do status é exatamente open/closed', () => {
    assert.match(migrationCode, /status\s+in\s*\(\s*'open'\s*,\s*'closed'\s*\)/i);
  });

  test('⭐⭐ a regra "uma sessão aberta por caixa" é um índice único parcial', () => {
    assert.match(
      migrationCode,
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('cashregister.session.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('cashregister.session.opened') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(cashregister\.[a-z.]+)'\)/g)].map((m) => m[1]);
