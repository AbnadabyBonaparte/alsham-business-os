...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-PDV-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'pdv',"));
  assert.ok(meu, 'o seed não registra o módulo pdv');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo pdv contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é VERTICAL `retail`, ancorado na linha de Varejo & Supermercados', () => {
    ...
    // A âncora é a linha que traz JUNTOS 'PDV' e 'Marketplace próprio'.
    ...
    // Cada capacidade do manifesto tem de estar nessa linha (separada por '·').
    const nomes = linha!.split('·').map((s) => s.trim());
    for (const c of MANIFEST.capabilities) {
      assert.ok(nomes.includes(c.canonicalName), `capacidade ${c.canonicalName} não está na linha da Taxonomia`);
    }
  });
  ...
  test('⛔ não há enum de status/pagamento — o total é VIEW, não coluna', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+pdv\./i);
    assert.match(migrationCode, /create\s+view\s+pdv\.sale_totals/i);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0086_pdv.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-PDV-SPEC.md não existe');
  });
});

describe('o schema é o esperado — cabeçalho + itens + total em VIEW', () => {
  test('não há CREATE TYPE em pdv (status/pagamento são texto/CHECK, nunca enum)', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+pdv\./i);
  });

  test('as duas tabelas do cupom existem', () => {
    assert.match(migrationCode, /create\s+table\s+pdv\.sales/i);
    assert.match(migrationCode, /create\s+table\s+pdv\.sale_items/i);
  });

  test('⭐ o total é VIEW calculada — nunca coluna', () => {
    assert.match(migrationCode, /create\s+view\s+pdv\.sale_totals/i);
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('pdv.sale.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('pdv.sale.registered') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(pdv\.[a-z.]+)'\)/g)].map((m) => m[1]);
