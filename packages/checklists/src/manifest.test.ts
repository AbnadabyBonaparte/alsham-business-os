...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-CHK-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'chk',"));
  assert.ok(meu, 'o seed não registra o módulo chk');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo chk contém ${needle}`);
  return JSON.parse(hit.slice(1, hit.lastIndexOf("'")));
}

describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ o Domain é `operations`, e Checklist está na linha dele', () => {
  ...
  /**
   * ⭐ ANTI-VIÉS com recusa nomeada: nada de cron de ronda, foto/anexo
   * (Storage do Core, não construído), assinatura digital, score de
   * inspeção — nem enum.
   */
  test('⛔ o schema não tem cron, foto, assinatura, score nem enum', () => {
    assert.doesNotMatch(migrationCode, /pg_cron|cron\.schedule|schedule_/i);
    assert.doesNotMatch(migrationCode, /photo|attachment|file_|upload/i);
    assert.doesNotMatch(migrationCode, /signature|assinatura_/i);
    assert.doesNotMatch(migrationCode, /score|scoring|nota_final/i);
    assert.doesNotMatch(migrationCode, /create\s+type\s+chk\./i);
  });

  test('⭐ o congelo é por CÓPIA, pelo gatilho — no CÓDIGO', () => {
    assert.match(migrationCode, /chk\.snapshot_template/);
    assert.match(migrationCode, /chk\.copy_items/);
    assert.match(migrationCode, /new\.template_name := v_name/);
  });

  test('⭐ concluir conta os pendentes — no CÓDIGO', () => {
    assert.match(migrationCode, /and answer is null/);
    assert.match(migrationCode, /pela metade/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0034_chk.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-CHK-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('chk.run.execute') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('chk.run.started') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(chk\.[a-z.]+)'\)/g)].map((m) => m[1]);
