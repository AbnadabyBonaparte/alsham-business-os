...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-WHISTLE-SPEC.md');
...
// seed ganhar a linha do `whistle`, os subtestes de TRANSCRIÇÃO abaixo falham DE
...
const blocoDoModulo: string | null = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'whistle',"));
  if (!meu) return null;
  return meu.slice(0, meu.indexOf('on conflict'));
})();

function jsonBlockContaining(needle: string): unknown[] {
  assert.ok(blocoDoModulo, 'o seed não registra o módulo whistle (cartão do orquestrador)');
  const blocks = blocoDoModulo.replace(/--[^\n]*/g, '').match(/'\[[\s\S]*?\]'::jsonb/g) ?? [];
  const hit = blocks.find((b) => b.includes(needle));
  assert.ok(hit, `nenhum bloco jsonb do módulo whistle contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('as permissões são EXATAMENTE submit e handle', () => {
    assert.deepEqual(
      ...
      ['whistle.report.handle', 'whistle.report.submit'],
    );
  });
  ...
  test('os eventos emitidos são EXATAMENTE os quatro do ciclo', () => {
    assert.deepEqual(
      ...
      [
        'whistle.report.dismissed',
        'whistle.report.registered',
        'whistle.report.resolved',
        'whistle.report.reviewed',
      ],
    );
  });

  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0092_whistle.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-WHISTLE-SPEC.md não existe');
  });
});

describe('⭐⭐ o anonimato mora no schema — a lei está no banco, não na tela', () => {
  test('a coluna is_anonymous existe (a denúncia sabe se é anônima)', () => {
    assert.match(migrationCode, /is_anonymous\s+boolean\s+not null/i);
  });

  test('⭐⭐ o gatilho de inserção DESCARTA o denunciante quando anônima', () => {
  ...
  test('⭐⭐ a CHECK constraint recusa anônima COM denunciante', () => {
    assert.match(
      migrationCode,
...
describe('⭐ o relato NÃO passeia no envelope — privacidade do payload', () => {
  function corpoDoPayload(): string {
    const corpo = migrationCode.split('create or replace function whistle.report_payload')[1];
    assert.ok(corpo !== undefined, 'whistle.report_payload não encontrada na migration');
    return corpo.split('$$;')[0] ?? '';
  }

  test('o payload leva SÓ metadado seguro: category, isAnonymous, status', () => {
    const corpo = corpoDoPayload();
    assert.match(corpo, /'category'/);
    assert.match(corpo, /'isAnonymous'/);
    assert.match(corpo, /'status'/);
  });

  test('⭐ o payload NÃO carrega o relato (description/subject) nem o denunciante', () => {
    const corpo = corpoDoPayload();
    ...
    assert.doesNotMatch(corpo, /reporter/i, 'o denunciante (reporter) vazou no envelope');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('whistle.report.submit') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('whistle.report.registered') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(whistle\.[a-z.]+)'\)/g)].map((m) => m[1]);
