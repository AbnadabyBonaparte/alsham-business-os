...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-IP-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'ip',"));
  if (!meu) return '';
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo ip contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⭐⭐ o tipo é CHECK das quatro categorias — não enum do produto', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+ip\./i);
    assert.match(
      migrationCode,
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0084_ip.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-IP-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('ip.asset.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('ip.asset.registered') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...codigo.matchAll(/\('(ip\.[a-z.]+)'\)/g)].map((m) => m[1]);
