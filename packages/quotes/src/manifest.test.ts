...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-QUOTE-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'quote',"));
  assert.ok(meu, 'o seed não registra o módulo quote');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo quote contém ${needle}`);
  return JSON.parse(hit.slice(1, hit.lastIndexOf("'")));
}

describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ o Domain é `crm`, e Propostas e Orçamentos estão na linha dele', () => {
  ...
  test('⚠️ Pipeline é capacidade da Taxonomia e NÃO é declarada — é o Módulo 10', () => {
    const declaradas: readonly string[] = MANIFEST.capabilities.map((c) => c.canonicalName);
    assert.equal(declaradas.includes('Pipeline'), false);
    assert.equal(declaradas.length, 2);
    // E o schema não tem estágio nem funil — proposta não anda em esteira.
    assert.doesNotMatch(migrationCode, /\bstage\b|\bfunnel\b|probability/i);
  });

  test('⭐ o schema não amarra a proposta a cadastro do crm — contraparte é NEUTRA', () => {
    // A pedreira 360° PRIMA algemava a oportunidade ao lead (lead_id NOT
    // NULL). Aqui NÃO entra: propor não exige cadastrar.
    assert.doesNotMatch(migrationCode, /lead_id|party_id|references\s+crm\./i);
  });
  ...
  test('todo evento emitido usa o prefixo do módulo e verbo no passado', () => {
    for (const e of MANIFEST.events.emits) {
      ...
      // `sent` é passado irregular (send → sent); os demais terminam em `ed`.
      // Os sete são fato consumado — nenhum é pedido.
      assert.match(
  ...
  test('os sete fatos encomendados existem todos', () => {
    const encomendados = [
      'proposal.registered',
      'proposal.updated',
      'proposal.sent',
    ...
    for (const f of encomendados) {
      assert.ok(emitidos.includes(`quote.${f}`), `o fato quote.${f} não é emitido`);
    }
    assert.equal(emitidos.length, 7);
  });

  test('não declara consumo sem consumidor construído (Lei 7)', () => {
    // O caminho aceite → título no ar está DECLARADO na spec (§5), não aqui.
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0024_quote.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-QUOTE-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('quote.proposal.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('quote.proposal.registered') as {
      type: string;
      version: number;
    }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(quote\.[a-z.]+)'\)/g)].map((m) => m[1]);
    assert.deepEqual(
      concedidas,
      [],
