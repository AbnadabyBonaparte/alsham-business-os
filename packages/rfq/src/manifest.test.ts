...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-RFQ-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'rfq',"));
  assert.ok(meu, 'o seed não registra o módulo rfq');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo rfq contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `procurement`, ancorado na linha de Compras da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Cotações') && l.includes('Estoque mínimo'));
  ...
  test('⛔ o schema não cria enum de item — item é texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+rfq\./i);
    assert.match(migrationCode, /item\s+text/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0059_rfq.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-RFQ-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('rfq.request.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('rfq.request.registered') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(rfq\.[a-z.]+)'\)/g)].map((m) => m[1]);
