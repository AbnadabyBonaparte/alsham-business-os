...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-NC-SPEC.md');
...
// ⚠️ O cartão do nc no seed é a ÚNICA peça que este módulo NÃO cria (o seed é
// arquivo compartilhado, editado pelo dono). Por isso a leitura do bloco é LAZY:
// só os testes do describe "o seed transcreve" a exercem e falham sozinhos até o
// cartão entrar — os testes de contrato do manifesto passam.
function blocoDoModulo(): string {
  const inserts = sql.split(/insert into core\.module_registry/);
  const meu = inserts.find((b) => b.includes("'nc',"));
  assert.ok(meu, 'o seed não registra o módulo nc (cartão adicionado pelo dono)');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo nc contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `quality`, ancorado na linha de Qualidade da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Não conformidades') && l.includes('CAPA'));
  ...
  test('⛔ o schema não cria enum de tipo — origem/descrição são texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+nc\./i);
    assert.match(migrationCode, /origin\s+text/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0078_nc.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-NC-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('nc.entry.register') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('nc.entry.registered') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(nc\.[a-z.]+)'\)/g)].map((m) => m[1]);
