...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-MALL-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'mall',"));
  assert.ok(meu, 'o seed não registra o módulo mall');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo mall contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⛔ o schema não recria cadastro de espaço nem enum de segmento — reaproveita o spc', () => {
    // A unidade física é id solto ao spc; nenhuma tabela de espaço nasce aqui.
    assert.doesNotMatch(migrationCode, /create\s+table\s+mall\.spaces/i);
    assert.doesNotMatch(migrationCode, /create\s+type\s+mall\./i);
    assert.match(migrationCode, /space_id/);
  });
  ...
  test('consumes é VAZIO — nenhum handler de shopping nesta onda (Lei 7)', () => {
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0053_mall.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-MALL-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('mall.store.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('mall.store.registered') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(mall\.[a-z.]+)'\)/g)].map((m) => m[1]);
