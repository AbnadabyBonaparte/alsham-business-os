...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-KANBAN-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'kanban',"));
  assert.ok(meu, 'o seed não registra o módulo kanban');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo kanban contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `pmo`, ancorado na linha de PMO & Projetos da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Kanban') && l.includes('Portfólio'));
  ...
  test('⛔ o schema não cria enum de etapa nem de status — texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+kanban\./i);
  ...
  test('⭐ a migration NÃO lê o schema do módulo Projetos (vínculo é id solto)', () => {
    assert.doesNotMatch(migrationCode, /\bproj\./, 'a migration referencia o schema proj — deve ser id solto');
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0070_kanban.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-KANBAN-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('kanban.board.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('kanban.stage.registered') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(kanban\.[a-z.]+)'\)/g)].map((m) => m[1]);
