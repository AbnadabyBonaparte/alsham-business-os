...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-BID-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'bid',"));
  assert.ok(meu, 'o seed não registra o módulo bid');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo bid contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é VERTICAL `government`, ancorado na linha de Governo da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Licitações') && l.includes('Fiscalização'));
  ...
  test('⛔ o schema não cria enum de item nem de modalidade — tudo texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+bid\./i);
    assert.match(migrationCode, /item\s+text/);
    assert.match(migrationCode, /modality\s+text/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0107_bid.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-BID-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('bid.tender.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('bid.tender.registered') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(bid\.[a-z.]+)'\)/g)].map((m) => m[1]);
