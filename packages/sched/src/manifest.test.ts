...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-SCHED-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'sched',"));
  assert.ok(meu, 'o seed não registra o módulo sched');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo sched contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `pmo`, ancorado na linha de PMO & Projetos da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Cronogramas') && l.includes('Portfólio'));
  ...
  test('⭐ a chave de domínio bate com a store-taxonomy (a seção PMO existe)', () => {
    ...
    assert.ok(
      store.includes("key: 'pmo'"),
      'store-taxonomy.ts não tem a chave pmo — a seção PMO não existiria',
    );
  });

  test('⛔ o schema não cria enum de tipo — a natureza é texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+sched\./i);
    assert.match(migrationCode, /title\s+text/);
  });

  test('🔴 a migration NÃO referencia o schema proj — o vínculo é id solto', () => {
    assert.doesNotMatch(migrationCode, /\bproj\./);
  });
  ...
  test('⚠️ o fato da conclusão é `completed` (status done não terminaria em -ed)', () => {
    const tipos = MANIFEST.events.emits.map((e) => e.type as string);
    assert.ok(tipos.includes('sched.milestone.completed'));
    assert.ok(!tipos.includes('sched.milestone.done'));
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0069_sched.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-SCHED-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('sched.milestone.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('sched.milestone.registered') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(sched\.[a-z.]+)'\)/g)].map((m) => m[1]);
