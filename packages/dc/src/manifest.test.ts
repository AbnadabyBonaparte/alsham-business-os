...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-DC-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'dc',"));
  assert.ok(meu, 'o seed não registra o módulo dc');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo dc contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⛔ o schema não cria enum de endereço — endereço é texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+dc\./i);
    assert.match(migrationCode, /address\s+text/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0065_dc.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-DC-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('dc.center.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('dc.center.registered') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(dc\.[a-z.]+)'\)/g)].map((m) => m[1]);
