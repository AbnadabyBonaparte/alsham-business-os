...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-PAT-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'pat',"));
  assert.ok(meu, 'o seed não registra o módulo pat');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo pat contém ${needle}`);
  return JSON.parse(hit.slice(1, hit.lastIndexOf("'")));
}

describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ o Domain é `operations`, e Patrimônio está na linha dele', () => {
  ...
  /**
   * ⭐ ANTI-VIÉS com recusa nomeada: nada de depreciação/vida útil (Lei 3 —
   * cálculo contábil), plano de manutenção (é do mnt), QR/leitor
   * (integração), nem coluna de localização vigente.
   */
  test('⛔ o schema não tem depreciação, plano de manutenção, QR nem enum', () => {
    assert.doesNotMatch(migrationCode, /deprecia|useful_life|vida_util/i);
    assert.doesNotMatch(migrationCode, /maintenance_plan|references\s+mnt\./i);
    assert.doesNotMatch(migrationCode, /qr_|barcode|rfid/i);
    assert.doesNotMatch(migrationCode, /create\s+type\s+pat\./i);
    assert.match(migrationCode, /create table pat\.categories/, 'a categoria é tabela do tenant');
    assert.match(migrationCode, /unique\s*\(tenant_id, lower\(btrim\(code\)\)\)|pat_assets_code_unique/, 'a etiqueta é única por tenant');
  });

  test('⭐ aquisição não mora no futuro, no CÓDIGO', () => {
    assert.match(migrationCode, /acquired_on is null or acquired_on <= current_date/);
  });

  test('⭐ o "de onde" da transferência é do servidor — e o livro é eterno', () => {
    assert.match(migrationCode, /new\.from_location\s*:=\s*pat\.current_location_of/);
    assert.match(migrationCode, /pat_transfers_immutable/);
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0033_pat.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-PAT-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('pat.asset.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('pat.asset.registered') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(pat\.[a-z.]+)'\)/g)].map((m) => m[1]);
