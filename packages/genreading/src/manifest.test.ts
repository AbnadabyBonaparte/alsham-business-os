...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-GENREADING-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'genreading',"));
  if (!meu) return '';
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo genreading contém ${needle}`);
  return JSON.parse(hit.slice(1, hit.lastIndexOf("'")));
}

describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é VERTICAL `energy`, ancorado na linha de Energia da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Monitoramento de geração') && l.includes('Comercialização e leads'));
  ...
  test('⭐⭐ o schema tem a tabela de leituras, o CHECK >= 0 e NÃO tem allowed_transition (livro imutável)', () => {
    assert.match(migrationCode, /create\s+table\s+genreading\.readings/i);
    assert.match(migrationCode, /check\s*\(\s*generated_kwh\s*>=\s*0\s*\)/i);
    // É livro imutável: nenhum ciclo de vida, nenhuma transição.
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0098_genreading.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-GENREADING-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('genreading.reading.record') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('genreading.reading.recorded') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...codigo.matchAll(/\('(genreading\.[a-z.]+)'\)/g)].map((m) => m[1]);
