...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-PARK-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'park',"));
  assert.ok(meu, 'o seed não registra o módulo park');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo park contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⛔ não existe status/enum — dentro/fora é implícito por exited_at', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+park\./i);
    assert.doesNotMatch(migrationCode, /status\s+text/i);
    assert.match(migrationCode, /exited_at/);
  });

  test('⭐ entered_at/exited_at são carimbados pelo SERVIDOR na migration (now()/auth.uid() no guard)', () => {
    assert.match(migrationCode, /new\.entered_at\s*:=\s*now\(\)/);
    assert.match(migrationCode, /new\.entered_by\s*:=\s*\(select auth\.uid\(\)\)/);
    assert.match(migrationCode, /new\.exited_at\s*:=\s*now\(\)/);
    assert.match(migrationCode, /new\.exited_by\s*:=\s*\(select auth\.uid\(\)\)/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0056_park.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-PARK-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('park.entry.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('park.entry.registered') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(park\.[a-z.]+)'\)/g)].map((m) => m[1]);
