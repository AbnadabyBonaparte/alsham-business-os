...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-PFOLIO-SPEC.md');
...
// ⚠️ O cartão do pfolio no seed é a ÚNICA peça que este módulo NÃO cria (o seed
// é arquivo compartilhado, editado pelo dono). Por isso a leitura do bloco é
...
function blocoDoModulo(): string {
  ...
  const meu = inserts.find((b) => b.includes("'pfolio',"));
  assert.ok(meu, 'o seed não registra o módulo pfolio (cartão adicionado pelo dono)');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo pfolio contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⛔ o schema não cria enum de tipo — nome/descrição são texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+pfolio\./i);
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0077_pfolio.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-PFOLIO-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('pfolio.portfolio.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('pfolio.portfolio.registered') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(pfolio\.[a-z.]+)'\)/g)].map((m) => m[1]);
