...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-DEM-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'dem',"));
  assert.ok(meu, 'o seed não registra o módulo dem');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo dem contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `supply-chain`, ancorado na linha de Supply Chain da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Planejamento de demanda') && l.includes('Performance logística'));
  ...
  test('⛔ o schema não cria enum de produto — produto é texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+dem\./i);
    assert.match(migrationCode, /product\s+text/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0063_dem.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-DEM-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('dem.plan.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('dem.plan.registered') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(dem\.[a-z.]+)'\)/g)].map((m) => m[1]);
