...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-SHIFT-SPEC.md');
...
const blocoDoModulo = (() => {
  // ⚠️ `module_id` 'shift' é único no seed (domain_key também é 'hr', dos
  // módulos irmãos comm/train/perf/pol, mas o module_id não colide).
  const inserts = sql.split(/insert into core\.module_registry/);
  const meu = inserts.find((b) => b.includes("'shift',"));
  assert.ok(meu, 'o seed não registra o módulo shift');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo shift contém ${needle}`);
  return JSON.parse(hit.slice(1, hit.lastIndexOf("'")));
}

describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ o Domain é `hr`, ancorado em Escalas — com as capacidades-irmãs da onda', () => {
    ...
    const linha = taxonomia.split('\n').find((l) => l.includes('Escalas'));
    assert.ok(linha, 'a linha de capacidades do Domain RH sumiu da Taxonomia');
    assert.ok(
      linha.includes('Admissão') && linha.includes('Treinamentos'),
      'as capacidades-irmãs da onda sumiram de RH',
    );
  });

  test('⛔ o schema não tem preço, recorrência nem enum de turno — a física é constraint', () => {
    assert.doesNotMatch(migrationCode, /price|amount_cents|billing_/i);
    assert.doesNotMatch(migrationCode, /recurrence|rrule/i);
    assert.doesNotMatch(migrationCode, /create\s+type\s+shift\./i);
    assert.match(migrationCode, /exclude using gist/);
    assert.match(migrationCode, /where \(status = 'scheduled'\)/);
  });

  test('⭐ nenhuma FK cruzada para hr.employees — o vínculo é ID SOLTO', () => {
    assert.doesNotMatch(migrationCode, /references\s+hr\.employees/i);
  });

  test('⭐ o passado é permitido — nenhuma trava de now()/current_date em starts_at', () => {
    assert.doesNotMatch(migrationCode, /starts_at\s*>=?\s*(now\(\)|current_date)/);
  });
  ...
  test('consumes é VAZIO — nenhum handler de Escalas nesta onda (Lei 7)', () => {
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0049_shift.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-SHIFT-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('shift.schedule.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('shift.schedule.scheduled') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(shift\.[a-z.]+)'\)/g)].map((m) => m[1]);
