...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-CONTINUITY-SPEC.md');
...
// ⚠️ O seed é arquivo compartilhado — esta onda NÃO o edita (o cartão do
// continuity entra no seed numa outra frente). Por isso a busca do bloco é
// RESILIENTE: se o módulo ainda não está no seed, `blocoDoModulo` é '' e SÓ os
// subtestes de "o seed transcreve o manifesto" falham — os do contrato do Core
// seguem verdes.
const blocoDoModulo = (() => {
  const inserts = sql.split(/insert into core\.module_registry/);
  const meu = inserts.find((b) => b.includes("'continuity',"));
  if (!meu) return '';
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo continuity contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `infosec`, ancorado na linha de Segurança da Informação da Taxonomia', () => {
    ...
    // canonicalName exato entre as capacidades do Domain.
  ...
  test('⭐ a chave de domínio bate com a store-taxonomy (a seção Segurança da Informação existe)', () => {
    ...
    assert.ok(
      store.includes("key: 'infosec'"),
      'store-taxonomy.ts não tem a chave infosec — a seção Segurança da Informação não existiria',
    );
  });

  test('⛔ o schema não cria enum de escopo/alvo — RTO/RPO são texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+continuity\./i);
    assert.match(migrationCode, /rto\s+text/);
    assert.match(migrationCode, /rpo\s+text/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0095_continuity.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-CONTINUITY-SPEC.md não existe');
  });
});

describe('⭐ o schema: o plano tem ciclo, o drill é imutável', () => {
  test('a tabela continuity.plans tem o CHECK active ↔ archived', () => {
    assert.match(migrationCode, /create\s+table\s+continuity\.plans/i);
    assert.match(migrationCode, /status\s+text[\s\S]*?check\s*\(\s*status\s+in\s*\(\s*'active'\s*,\s*'archived'\s*\)/i);
  });

  test('⭐⭐ a tabela continuity.drills é IMUTÁVEL (gatilho guard_drill_immutable, before update or delete)', () => {
    assert.match(migrationCode, /create\s+table\s+continuity\.drills/i);
    assert.match(migrationCode, /before\s+update\s+or\s+delete\s+on\s+continuity\.drills/i);
    assert.match(migrationCode, /guard_drill_immutable/);
    assert.match(migration, /fato consumado/);
  });

  test('⭐ só grant select, insert no livro de drills — o cliente não tem porta de reescrita', () => {
    assert.match(migration, /grant\s+select,\s*insert\s+on\s+continuity\.drills\s+to\s+authenticated/i);
    assert.doesNotMatch(migration, /grant[^;]*\bupdate\b[^;]*on\s+continuity\.drills\s+to\s+authenticated/i);
    assert.doesNotMatch(migration, /grant[^;]*\bdelete\b[^;]*on\s+continuity\.drills\s+to\s+authenticated/i);
    assert.doesNotMatch(migration, /create\s+policy[\s\S]*?for\s+update\s+on\s+continuity\.drills/i);
    assert.doesNotMatch(migration, /create\s+policy[\s\S]*?for\s+delete\s+on\s+continuity\.drills/i);
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('continuity.plan.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('continuity.plan.registered') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(continuity\.[a-z.]+)'\)/g)].map((m) => m[1]);
