...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-SUBSCRIPTION-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'subscription',"));
  assert.ok(meu, 'o seed não registra o módulo subscription');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo subscription contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é VERTICAL `energy`, ancorado na linha de Energia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Assinatura de energia') && l.includes('Comercialização e leads'));
  ...
  test('⭐⭐ UM módulo, a capacidade Assinatura de energia', () => {
    const keys = MANIFEST.capabilities.map((c) => c.key);
    assert.ok(keys.includes('energy-subscription'), 'falta a capacidade Assinatura de energia');
  });

  test('⛔ a tabela de assinaturas existe, com o CHECK de status e de alocação', () => {
    assert.match(migrationCode, /create\s+table\s+subscription\.subscriptions/i);
    assert.match(migrationCode, /check\s*\(\s*status in \('active', 'cancelled'\)\s*\)/i);
    assert.match(migrationCode, /allocation_percent > 0 and allocation_percent <= 100/i);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0097_subscription.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-SUBSCRIPTION-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('subscription.subscription.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('subscription.subscription.registered') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...codigo.matchAll(/\('(subscription\.[a-z.]+)'\)/g)].map((m) => m[1]);
