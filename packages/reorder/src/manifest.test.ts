...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-REORDER-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'reorder',"));
  assert.ok(meu, 'o seed não registra o módulo reorder');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo reorder contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `procurement`, ancorado na linha de Compras da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Estoque mínimo') && l.includes('Fornecedores'));
  ...
  test('⛔ o schema não cria enum — status é CHECK, quantidade mínima é numeric', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+reorder\./i);
    assert.match(migrationCode, /minimum_quantity\s+numeric/);
  });

  test('⭐⭐ a migration NÃO lê o schema `inv` — módulo não conhece módulo', () => {
    assert.doesNotMatch(migrationCode, /\binv\./i);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0062_reorder.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-REORDER-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('reorder.rule.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('reorder.rule.registered') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(reorder\.[a-z.]+)'\)/g)].map((m) => m[1]);
