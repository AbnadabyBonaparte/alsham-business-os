...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-TIMESHEET-SPEC.md');
...
// timesheet entra no seed numa outra frente). Por isso a busca do bloco é
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'timesheet',"));
  if (!meu) return '';
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo timesheet contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⛔ o schema não cria enum de tipo — nome/descrição são texto livre', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+timesheet\./i);
    assert.match(migrationCode, /collaborator_name\s+text/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0076_timesheet.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-TIMESHEET-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('timesheet.entry.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('timesheet.entry.registered') as { type: string }[];
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...codigo.matchAll(/\('(timesheet\.[a-z.]+)'\)/g)].map((m) => m[1]);
