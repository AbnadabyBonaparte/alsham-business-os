...
const MIGRATION = resolve(HERE, '../../../supabase/migrations/0055_fund.sql');
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-FUND-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'fund',"));
  assert.ok(meu, 'o seed não registra o módulo fund');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo fund contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  ...
  test('⛔ o schema não importa o cc — livro próprio, autossuficiente', () => {
    assert.doesNotMatch(migrationCode, /create\s+schema\s+if\s+not\s+exists\s+cc/i);
    assert.doesNotMatch(migrationCode, /cc\.execute_rateio/);
    assert.doesNotMatch(migrationCode, /\bfrom\s+cc\./i);
  });
  ...
  test('consumes é VAZIO — quem contribui e gasta é gente (Lei 7)', () => {
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0055_fund.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-FUND-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('fund.contribution.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('fund.contribution.recorded') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(fund\.[a-z.]+)'\)/g)].map((m) => m[1]);
