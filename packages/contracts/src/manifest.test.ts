...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-CTR-SPEC.md');
const TAXONOMIA = resolve(HERE, '../../../docs/canon/TAXONOMIA-EMPRESARIAL-ALSHAM.md');
const sql = readFileSync(SEED, 'utf8');
const migration = readFileSync(MIGRATION, 'utf8');

// A lição das guardas, mais uma vez: confere-se o CÓDIGO, não a prosa.
const migrationCode = migration.replace(/--[^\n]*/g, '');

const blocoDoModulo = (() => {
  const inserts = sql.split(/insert into core\.module_registry/);
  const meu = inserts.find((b) => b.includes("'ctr',"));
  assert.ok(meu, 'o seed não registra o módulo ctr');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo ctr contém ${needle}`);
  return JSON.parse(hit.slice(1, hit.lastIndexOf("'")));
}

describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ o Domain é `legal`, e Contratos está na linha dele', () => {
    const taxonomia = readFileSync(TAXONOMIA, 'utf8');
    assert.equal(MANIFEST.taxonomy.layer, 'domain');
    assert.equal(MANIFEST.taxonomy.domain, 'legal');
    const linha = taxonomia.split('\n').find((l) => l.includes('Contratos · Procurações'));
    assert.ok(linha, 'a linha de capacidades do Domain Jurídico sumiu da Taxonomia');
  ...
  /**
   * ⭐ ANTI-VIÉS com recusa nomeada: o que NÃO entrou, conferido no CÓDIGO
   * do schema — assinatura (Engine), arquivo/PDF (Storage do Core), enum de
   * tipo e de índice (vocabulário de uma casa), juros/multa (é do dun/ar).
   */
  test('⛔ o schema não tem assinatura, arquivo, enum de tipo nem juros', () => {
    assert.doesNotMatch(migrationCode, /\bsignature\b|\bsigned_url\b|docusign/i);
    assert.doesNotMatch(migrationCode, /\bpdf\b|file_url|attachment/i);
    assert.doesNotMatch(migrationCode, /create\s+type\s+ctr\./i);
    assert.doesNotMatch(migrationCode, /\b(igpm|igp_m|ipca|selic)\b/i, 'índice virou coluna/enum — é texto livre');
    assert.doesNotMatch(migrationCode, /interest|penalty|\bfine\b/i);
  });
  ...
  test('⭐ o sistema não calcula índice: nenhuma função de cálculo de reajuste', () => {
    // O ato REGISTRA valores decididos; não existe percentual nem fator.
    assert.doesNotMatch(migrationCode, /percent|factor|rate\b/i);
  });
  ...
  test('consumes é VAZIO — e é decisão declarada, não esquecimento', () => {
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0028_ctr.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-CTR-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('ctr.contract.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('ctr.contract.registered') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(ctr\.[a-z.]+)'\)/g)].map((m) => m[1]);
