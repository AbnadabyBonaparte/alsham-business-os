...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-DISP-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'disp',"));
  assert.ok(meu, 'o seed não registra o módulo disp');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo disp contém ${needle}`);
...
describe('o manifesto obedece ao contrato do Core', () => {
  test('⭐ é DOMAIN `supply-chain`, ancorado na linha de Supply Chain da Taxonomia', () => {
    ...
    const linha = taxonomia
      .split('\n')
      .find((l) => l.includes('Distribuição') && l.includes('Performance logística'));
  ...
  test('⛔ o schema não cria enum nem coluna de status — o despacho é ato pontual', () => {
    assert.doesNotMatch(migrationCode, /create\s+type\s+disp\./i);
    assert.doesNotMatch(migrationCode, /status\s+text/i);
    assert.match(migrationCode, /quantity\s+numeric/);
  });

  test('🔴 a migration NÃO referencia o schema de centros de distribuição (vínculo id solto)', () => {
    assert.doesNotMatch(migrationCode, /dc\./i);
    assert.match(migrationCode, /dc_center_id\s+uuid/i);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0066_disp.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-DISP-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('disp.dispatch.record') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('disp.dispatch.recorded') as { type: string }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(disp\.[a-z.]+)'\)/g)].map((m) => m[1]);
