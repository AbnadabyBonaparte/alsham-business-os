...
const SPEC = resolve(HERE, '../../../docs/canon/MODULO-CC-SPEC.md');
...
const blocoDoModulo = (() => {
  ...
  const meu = inserts.find((b) => b.includes("'cc',"));
  assert.ok(meu, 'o seed não registra o módulo cc');
...
function jsonBlockContaining(needle: string): unknown[] {
  ...
  assert.ok(hit, `nenhum bloco jsonb do módulo cc contém ${needle}`);
  return JSON.parse(hit.slice(1, hit.lastIndexOf("'")));
}

describe('o manifesto obedece ao contrato do Core', () => {
  /**
   * ⭐ O Domain é `finance` — a Taxonomia §5 põe *Centro de custo* e
   * *Rateio* na linha do 💰 Financeiro, a mesma casa do cash.
   */
  test('⭐ o Domain é `finance` — e a linha do Financeiro tem Centro de custo e Rateio', () => {
    ...
    const linha = taxonomia.split('\n').find((l) => l.includes('Fluxo de caixa'));
    assert.ok(linha, 'a linha do Domain Financeiro sumiu da Taxonomia');
    assert.ok(linha.includes('Centro de custo') && linha.includes('Rateio'), 'as capacidades sumiram da linha');
  });

  /**
   * ⭐ ANTI-VIÉS com recusa nomeada: nada de cron/rateio por relógio,
   * integração contábil, plano de centros semeado nem vocabulário de
   * vertical — nem enum: centro é dado do tenant.
   */
  test('⛔ o schema não tem cron, plano semeado, vocabulário de vertical nem enum', () => {
    assert.doesNotMatch(migrationCode, /pg_cron|cron\.schedule|scheduled_for/i);
    assert.doesNotMatch(migrationCode, /shopping|mall|loja|inquilino|lojista/i);
    assert.doesNotMatch(migrationCode, /create\s+type\s+cc\./i);
    // A física do 100% e a origem por id solto estão no arquivo.
    assert.match(migrationCode, /v_sum <> 10000/);
    assert.match(migrationCode, /source_ref\s+uuid,/);
  });
  ...
  test('a migration e a spec do módulo existem', () => {
    assert.ok(existsSync(MIGRATION), '0043_cc.sql não existe');
    assert.ok(existsSync(SPEC), 'MODULO-CC-SPEC.md não existe');
  });
});
...
describe('o seed transcreve o manifesto fielmente', () => {
  ...
  test('as permissões do seed são exatamente as do manifesto', () => {
    const seeded = jsonBlockContaining('cc.center.manage') as { key: string }[];
  ...
  test('os eventos emitidos do seed são exatamente os do manifesto', () => {
    const seeded = jsonBlockContaining('cc.rateio.executed') as { type: string; version: number }[];
    assert.deepEqual(
  ...
  test('⛔ seed NÃO concede permissão de módulo — quem concede é o instalador', () => {
    ...
    const concedidas = [...code.matchAll(/\('(cc\.[a-z.]+)'\)/g)].map((m) => m[1]);
