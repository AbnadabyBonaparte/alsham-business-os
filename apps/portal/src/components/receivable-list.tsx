...
/**
 * A lista de títulos a receber — o rosto do Módulo 5.
 *
 * ⭐ **Este componente não decide nada.** Vencido, saldo e excedente são
 * perguntas feitas a `@alsham/accounts-receivable`. Se alguém escrever
 * `t.amountCents - t.receivedAmountCents` aqui, a regra passa a existir em dois
 * lugares que vão divergir — e o segundo deles devolveria número NEGATIVO no
 * caso de recebimento a maior, que é justamente o que o pacote evita.
 *
 * Cancelar tem **confirmação explícita em dois passos** (padrão CRIVO).
 */
  ...
  return (
    <Panel className="px-6 py-5">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-baseline gap-2">
            <h2 className="font-display text-lg text-bos-text">
              {titulo.payerName ?? 'Sem pagador nomeado'}
            </h2>
          ...
          <p className="mt-2 text-xs text-bos-muted">
            vence em {shortDate(titulo.dueDate)} · {money(titulo.amountCents, titulo.currency)} a
            receber
            {titulo.receivedAmountCents > 0 ? (
              <> · {money(titulo.receivedAmountCents, titulo.currency)} recebido</>
            ) : null}
            {saldo > 0 && titulo.receivedAmountCents > 0 ? (
          ...
          {/* ⭐ O excedente aparece EM VOZ ALTA, e é a divergência do módulo
              chegando à tela. Um excedente invisível é um número que não bate e
              ninguém sabe por quê. */}
          {excedente > 0 ? (
            <p className="mt-2 max-w-2xl text-xs text-bos-muted">
              Entrou <strong className="text-bos-text">{money(excedente, titulo.currency)}</strong>{' '}
              a mais do que o devido. Não é erro: o pagador pode ter arredondado, incluído encargos
              ou quitado mais de um documento numa transferência só. O excedente é crédito dele — e
              o que fazer com ele é a capacidade <em>Cobrança</em>, que não está construída.
            </p>
          ) : null}
        </div>
      ...
      <div className="mt-4 border-t border-bos-border pt-4">
        {podeCancelar ? (
          <CancelButton receivableId={titulo.id} canCancelReceivables={canCancelReceivables} />
        ) : (
