...
-- ---------------------------------------------------------------------------
-- O MÓDULO `esg` (Métricas Ambientais) — o 67º cartão. ABRE a Onda Quinze e o
-- Domain 🌱 ESG & Sustentabilidade. UM módulo para as quatro capacidades de
-- medição (carbono/água/energia/resíduo), unificadas por metric_type (CHECK).
-- Indicadores ESG são o goal e Relatórios ESG são o pol — ambos FORA.
-- ---------------------------------------------------------------------------
insert into core.module_registry (
  module_id, name, version, summary, layer, domain_key,
  capabilities, permissions, events_emits, events_consumes, agents,
...
values (
  ...
  'domain', 'esg',
  '[
     {"key":"esg","canonicalName":"Inventário de carbono"}
   ]'::jsonb,
  '[
     {"key":"esg.reading.record","moduleId":"esg","description":"Registrar uma leitura ambiental — o tipo da métrica, a quantidade, a unidade e o período."}
   ]'::jsonb,
  '[
     {"type":"esg.reading.recorded","version":1,"description":"Uma leitura ambiental foi registrada. Ato imutável desde o instante 1."}
   ]'::jsonb,
  '[]'::jsonb,
  '[]'::jsonb,
  '0.0.x',
