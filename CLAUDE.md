...
- ✅ **Ainda são só ARQUIVO** — aplicar é ato do dono:
  ...
  - `0120_secret_pages.sql` — as PÁGINAS RESERVADAS (Core, não módulo): a prateleira `core.secret_pages` (slug aleatório, `tenant_id`, título, corpo markdown) + a leitora `core.read_secret_page()` (`security definer`, vínculo checado na 1ª linha, molde do 0116). ⚠️ **Nenhum schema novo** (`core`, já exposto). ✅ **NÃO consome evento** — SEM redeploy do `apps/api`. ⛔⛔ **§3 resolvido pela raiz:** o CONTEÚDO (nome, proposta) o dono insere DIRETO no banco depois do apply — **nunca em código, commit ou branch**; o repositório só carrega a estrutura genérica. Duas camadas: login no tenant (a porta devolve `null` sem sessão) + o slug exato (a RLS resolve só dentro do próprio tenant). A rota `/p/[slug]` do portal não aparece em menu, busca (Ctrl+K) nem sitemap (guarda de CI). O catálogo **não muda**.
...
supabase/migrations/   0001_core … 0014_ap_apply_recon_match
                       ...
                       0120_secret_pages                   — PÁGINAS RESERVADAS
                                                            (Core, não módulo:
                                                            core.secret_pages —
                                                            conteúdo por slug, do
                                                            tenant, no banco; §3:
                                                            nome nunca em código,
                                                            só no banco); apply
                                                            do dono
                       (lacuna 0015–0016 proposital; 0119 RESERVADA ao PR do
                        fuso do tenant, aberto à parte; próxima livre: 0121)
